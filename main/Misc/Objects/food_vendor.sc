...
LVAR_INT food_stand askedFlag dobreak timetoEnd 
LVAR_INT flag vendor timer ivarsay 
askedFlag = 0
flag = 0
dobreak = 0
timetoEnd = 0
IF flag = 1
    CREATE_OBJECT_NO_OFFSET gb_hotdogstand01   0.0 0.0 0.0 food_stand
    ...
    IF DOES_OBJECT_EXIST food_stand
		if IS_OBJECT_WITHIN_BRAIN_ACTIVATION_RANGE food_stand
			IF IS_PLAYER_PLAYING player1
				...
				AND dobreak = 0
					IF flag = 0
						GET_OFFSET_FROM_OBJECT_IN_WORLD_COORDS food_stand 0.0 -0.7 -1.0 x y z
						x_temp = x - 0.5
						y_temp = y - 0.5
						z_temp = z
						temp_float_1 = x + 0.5
						...
						if not IS_AREA_OCCUPIED x_temp y_temp z_temp temp_float_1 temp_float_2 temp_float_3 false false true false false
							if not IS_point_on_screen x y z 1.0
								...
								if has_model_loaded BMOCHIL
									...
									GET_OFFSET_FROM_OBJECT_IN_WORLD_COORDS food_stand 0.0 0.8 0.0 x_temp y_temp z_temp
									x_temp -= x
									y_temp -= y
					...
					IF flag = 1
						IF LOCATE_CHAR_ON_FOOT_OBJECT_2D scplayer food_stand 8.0 8.0 0
							IF askedFlag = 0 
							AND LOCATE_CHAR_ON_FOOT_OBJECT_2D scplayer food_stand 3.0 3.0 0
								SET_CHAR_SAY_CONTEXT_IMPORTANT vendor CONTEXT_GLOBAL_WHAT_WANT 	TRUE		FALSE	FALSE ivarsay
								//SET_CHAR_SAY_CONTEXT_IMPORTANT vendor    CONTEXT_GLOBAL_WELCOME_TO_SHOP 	TRUE		FALSE	FALSE ivarsay
								askedFlag = 1
							ENDIF
							REQUEST_ANIMATION VENDING
							IF HAS_ANIMATION_LOADED VENDING
								IF IS_SCORE_GREATER player1 0
									GET_OFFSET_FROM_OBJECT_IN_WORLD_COORDS food_stand 0.0 1.0 0.5 x y z
									//GET_LEVEL_DESIGN_COORDS_FOR_OBJECT food_stand 1 x y z
									IF LOCATE_CHAR_ON_FOOT_3D scplayer x y z 1.0 1.0 2.0 TRUE
										GET_CHAR_HEALTH scplayer temp_integer_1
										temp_integer_1 += 50
										SET_CHAR_HEALTH scplayer temp_integer_1
										//SET_CHAR_SAY_CONTEXT vendor CONTEXT_GLOBAL_TAKE_A_SEAT  ivarsay
										SET_CHAR_SAY_CONTEXT_IMPORTANT vendor  CONTEXT_GLOBAL_GIVE_PRODUCT	TRUE		FALSE	FALSE ivarsay
										//LVAR_INT drinks_can
					...
					IF flag = 3
						if timer < game_timer
							GET_OFFSET_FROM_OBJECT_IN_WORLD_COORDS food_stand 0.0 0.2 0.0 x y z
					...
					ENDIF
					IF flag > 0 
					AND  HAS_CHAR_BEEN_DAMAGED_BY_WEAPON vendor WEAPONTYPE_ANYWEAPON
						dobreak = 1
						//WRITE_DEBUG AAAAAAAAA
						timetoEnd = game_timer + 40000
					ENDIF
				else
					IF flag > 0
